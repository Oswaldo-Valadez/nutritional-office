...
import "../App.css";
import { calcICC, calcIMC, getICCResult, getIMCResult } from "../utils/functions";
...
function NuevaNota() {
    ...
    useEffect(() => {
        const getPaciente = () => {
            axios.get('/expedients/' + id_expedient).then(res => {
                setFormulario({ ...formulario, paciente: res.data.expedient });
            })
                .catch(err => console.log(err))
        }
        const getNotaAnterior = () => {
            axios.get('notas/ultimaNota/' + id_expedient).then(res => {
    ...
    const handleSubmit = (e) => {
        ...
        axios.post('/notas/crearNota', form).then(res => {
                            ...
                            <Grid item sm={8}>
                                <Paper variant="outlined" style={{ padding: 30 }}>
                                    ...
                                    <Grid container spacing={2}>
                                        <Grid item xs={12}>
                                            <Typography variant='h6'>{formulario.paciente.fullname}</Typography>
                                        </Grid>
                                    ...
                                    {ultimaNota ? (
                                        <div>
                                            <Typography gutterBottom>
                                                <b>Fecha: </b>{moment(ultimaNota.register_date).format("DD/MM/YYYY")}
                                            </Typography>
                                            <Typography gutterBottom>
                                                <b>Peso: </b>{ultimaNota.weight}kg
                                            </Typography>
                                            <Typography gutterBottom>
                                                <b>Talla: </b>{ultimaNota.size}m
                                            </Typography>
                                            <Typography gutterBottom>
                                                <b>Cintura: </b>{ultimaNota.waist}cm
                                            </Typography>
                                            <Typography gutterBottom>
                                                <b>Cadera: </b>{ultimaNota.hip}cm
                                            </Typography>
                                            <Typography gutterBottom>
                                                <b>Presión arterial alta: </b>{ultimaNota.blood_pressure_high}mmHg
                                            </Typography>
                                            <Typography gutterBottom>
                                                <b>Presión arterial baja: </b>{ultimaNota.blood_pressure_low}mmHg
                                            </Typography>
                                            <Typography gutterBottom>
                                                <b>IMC: </b>{ultimaNota.bmi} - {getIMCResult(ultimaNota.bmi)}
                                            </Typography>
                                            <Typography gutterBottom>
                                                <b>ICC: </b>{ultimaNota.whr} - {getICCResult(ultimaNota.whr, ultimaNota.waist, formulario.paciente.gender)}
                                            </Typography>
                                            <Typography gutterBottom>
                                                <b>Anotaciones: </b>{ultimaNota.annotations}
                                            </Typography>
                                        </div>
                                    )
